...
DECLARE
    seq_record RECORD;
BEGIN
    FOR seq_record IN SELECT sequence_schema, sequence_name FROM information_schema.sequences WHERE sequence_schema = 'public'
    LOOP
        EXECUTE 'DROP SEQUENCE ' || quote_ident(seq_record.sequence_schema) || '.' || quote_ident(seq_record.sequence_name) || ' CASCADE;';
    END LOOP;
END $$;
...
-- Creating sequences for each table and setting the start value just above the highest current ID
...
SELECT setval('products_productid_seq', COALESCE((SELECT MAX(ProductID)+1 FROM Products), 1), false);

-- Sequence for Orders
CREATE SEQUENCE orders_orderid_seq;
SELECT setval('orders_orderid_seq', COALESCE((SELECT MAX(OrderID)+1 FROM Orders), 1), false);

-- Sequence for OrderDetails
CREATE SEQUENCE orderdetails_orderdetailid_seq;
SELECT setval('orderdetails_orderdetailid_seq', COALESCE((SELECT MAX(OrderDetailID)+1 FROM OrderDetails), 1), false);

-- Sequence for Categories
CREATE SEQUENCE categories_categoryid_seq;
SELECT setval('categories_categoryid_seq', COALESCE((SELECT MAX(CategoryID)+1 FROM Categories), 1), false);

-- Sequence for Shippers
CREATE SEQUENCE shippers_shipperid_seq;
SELECT setval('shippers_shipperid_seq', COALESCE((SELECT MAX(ShipperID)+1 FROM Shippers), 1), false);

-- Sequence for Suppliers
CREATE SEQUENCE suppliers_supplierid_seq;
SELECT setval('suppliers_supplierid_seq', COALESCE((SELECT MAX(SupplierID)+1 FROM Suppliers), 1), false);

-- Sequence for Employees
CREATE SEQUENCE employees_employeeid_seq;
SELECT setval('employees_employeeid_seq', COALESCE((SELECT MAX(EmployeeID)+1 FROM Employees), 1), false);
...
ALTER TABLE orders
    ADD CONSTRAINT fk_shipperid FOREIGN KEY (ShipperID) REFERENCES Shippers(ShipperID);

-- Adding foreign keys to the 'order_details' table
...
-- Adding foreign keys to the 'products' table
...
-- Set default sequence values for existing tables using ALTER TABLE

-- Alter Customers table to use the sequence for CustomerID
ALTER TABLE Customers ALTER COLUMN CustomerID SET DEFAULT nextval('customers_customerid_seq');

-- Alter Products table to use the sequence for ProductID
ALTER TABLE Products ALTER COLUMN ProductID SET DEFAULT nextval('products_productid_seq');

-- Alter Orders table to use the sequence for OrderID
ALTER TABLE Orders ALTER COLUMN OrderID SET DEFAULT nextval('orders_orderid_seq');

-- Alter OrderDetails table to use the sequence for OrderDetailID
ALTER TABLE OrderDetails ALTER COLUMN OrderDetailID SET DEFAULT nextval('orderdetails_orderdetailid_seq');

-- Alter Categories table to use the sequence for CategoryID
ALTER TABLE Categories ALTER COLUMN CategoryID SET DEFAULT nextval('categories_categoryid_seq');

-- Alter Shippers table to use the sequence for ShipperID
ALTER TABLE Shippers ALTER COLUMN ShipperID SET DEFAULT nextval('shippers_shipperid_seq');

-- Alter Suppliers table to use the sequence for SupplierID
ALTER TABLE Suppliers ALTER COLUMN SupplierID SET DEFAULT nextval('suppliers_supplierid_seq');

-- Alter Employees table to use the sequence for EmployeeID
ALTER TABLE Employees ALTER COLUMN EmployeeID SET DEFAULT nextval('employees_employeeid_seq');



SELECT 'CREATED W3Schools' AS Status;
